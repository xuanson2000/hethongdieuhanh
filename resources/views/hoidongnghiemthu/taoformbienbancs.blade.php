...
<div class="container" style="background-color: white; padding: 20px 20px 20px 30px; width: 100%;">
	...
	<div class="row" style=" border-style: groove;">
		<?php 
		$nghiemthucapcosoid=App\nghiemthucapcoso::find($id_ntcs);
		$nhiemvu_donvi=App\nhiemvu_donvithuchien::find($nghiemthucapcosoid->id_nhiemvu_donvi);
		$phongban=App\phongban::find($nhiemvu_donvi->id_donvi);
		?>

		<form action="{{route('saveformbienban',[$id_ntcs,$loaint])}}" method="POST" enctype="multipart/form-data">
			<input type="hidden"  name="_token" value="{{csrf_token()}}" >
			<div class="col-md-6" style="margin-top: 20px;">
				<center><p style="font-weight:bold; margin-bottom: -2px;">{{$phongban->tenviethoa}}</p> 
				...
				<table class="table table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td>@if($phuluckhoiluongtt->khoiluong!=null)  <input type="number" step="0.01" class="form-control" name="thcs[]" />   @endif</td>
							<td>@if($phuluckhoiluongtt->khoiluong!=null)  <input type="number"  step="0.01" class="form-control" name="ntcs[]" />   @endif</td>
