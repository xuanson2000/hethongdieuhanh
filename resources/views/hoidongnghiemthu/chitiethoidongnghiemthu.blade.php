...
<div class="container" style="background-color: white; padding: 20px 20px 20px 30px; width: 100%;">
	...
	<div class="model">
    	...
    	// $phuluckhoiluongtts=App\phuluckhoiluongtt::where('id_nhiemvu_donvi',$nhiemvu_donvithuchienid->id)->orderBy('id','ASC')->get();

    	if($nghiemthucapquanlyid->loainghiemthu==6){
    		$phuluckhoiluongtts=App\phuluckhoiluongtt::where('id_nhiemvu_donvi',$nhiemvu_donvithuchienid->id)->where('id_chuyengiaql6t',[Auth::guard('quantri')->user()->id])->orderBy('id','ASC')->get();

    	}else{

    		$phuluckhoiluongtts=App\phuluckhoiluongtt::where('id_nhiemvu_donvi',$nhiemvu_donvithuchienid->id)->where('id_chuyengiaql12t',[Auth::guard('quantri')->user()->id])->orderBy('id','ASC')->get();
    	}
    	...
    	<table class="table table-bordered">
    		...
    		<tbody>
    			<tr>
    				...
    				<td> <a href="public/tailieu/{{$nhiemvuchuyenmon->file_quyetdinh}}">file_quyetdinh </a> </td>
    				<td> <a href="public/tailieu/{{$nhiemvuchuyenmon->file_decuong}}">file_quyetdinh </a> </td>
    				<td> <a href="public/tailieu/{{$nhiemvuchuyenmon->file_hosoquyetdinh}}">file_quyetdinh </a> </td>
    				<td><a href="public/tailieu/{{$nhiemvuchuyenmon->file_thamdinh}} ">file_quyetdinh </a> </td>
		...
		<table class="table table-bordered">
    		...
    		<tbody>
    			<tr>
    				...
    				<td> <a href="public/tailieu/{{$nhiemvu_donvithuchienid->file}}">file_quyetdinh </a> </td>
    	...
    	<h5 style="font-weight:bold;"> <i class="fa fa-check-square-o" aria-hidden="true"></i> KẾT QUẢN NGHIỆM THU</h5>

    	<table class="table table-bordered">
    		<thead>
    			<tr>
    				<th style="width:30%;">Biên bản nghiệm thu</th>
    				<th> 
                      <a title="Gửi biên bản nghiệm thu" href=" {{route('taoformbienban',[$nhiemvu_donvithuchienid->id,$nghiemthucapquanlyid->loainghiemthu,$nghiemthucapquanlyid->id])}}" style="float:right;"> <i class="fa fa-plus-square" aria-hidden="true"></i> Tạo biên bản nghiệm thu</a>
                      
    					<!-- <a style="float:right; color: #FFA303; margin-right: 10px;"  title="Mẫu biên bản nghiệm thu đầu năm" href="public/anh/bienbanngiemthuquan6.docx"><i class="fa fa-cloud-download" aria-hidden="true"></i></a>

    					<a  style="float:right; color:#24C41C; margin-right: 10px;" title="Mẫu biên bản nghiệm thu cuối năm" href="public/anh/bienbanngiemthuquan12.docx"><i class="fa fa-cloud-download" aria-hidden="true"></i></a> -->
	...
	<div class="modal-dialog"style="width:60%;">
		<div class="modal-content">
			...
			<div class="" style="background-color: #4F78F4; height: 40px;">
				<h4 style="float: left; margin: 10px 10px 10px 10px; color: white; font-family: Helvetica;"><i class="fa fa-check-square-o" aria-hidden="true"></i> Gửi biên bản nghiệm thu</h4>
			...
			<div class="modal-body">
				<form action="{{route('guibienbannghiemthu',[$nghiemthucapquanly_chuyengia->id])}}" method="POST" enctype="multipart/form-data">
