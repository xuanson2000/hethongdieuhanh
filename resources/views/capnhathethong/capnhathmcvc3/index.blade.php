  ...
  {!! $songnhos->appends(Request::all())->links() !!}
   ...
   <table class="table table-bordered">
   	<thead>
   		<tr>
   			<th>STT</th>
   			<th>TÊN HM CÔNG VIỆC C3</th>
   			<th>THUỘC HẠNG MỤC C2</th>
   			<th>THAO TÁC</th>
   		</tr>
   	</thead>
   	...
   	<tbody>
   		...
   		<tr>
   			<td style="text-align: center;">{{$loop->index+1}}</td>
   			<td>{{$hmcv_cap3->tenhmcv}}</td>
   			<?php $tenhmcv2=DB::table('hmcv_cap2')->find($hmcv_cap3->id_hmcv_c2); ?>
   			<td>   @if($tenhmcv2 !=null) {{$tenhmcv2->tenhmcv}} @endif</td>
   			<td style="text-align: center;"> 
   				<a title="Sửa" href=""><i class="fa fa-pencil" aria-hidden="true"></i> </a>
   				
   				<a title="Xóa" onclick="return xacnhanxoa('Bạn có chắc chắn muốn xóa không')"  href="{{route('xoahmcvc3',[$hmcv_cap3->id])}}"><i class="fa fa-trash" aria-hidden="true"></i></a>
   			</td>
   		</tr>
   		@endforeach
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
        	...
        	<form action="{{route('themhmcvc3')}}" method="POST" enctype="multipart/form-data">
        		<input type="hidden"  name="_token" value="{{csrf_token()}}" >

        		<div class=" form-group" style="">
        			<label style="font-weight: bold; float:left;">Hạng mục cv cấp 2 <span style="color: red;">(*)</span></label>
        			<select class="form-control" name="id_hmcv_c2" required>
        				<option value="">Chọn hmcv c2</option>
        				@foreach($hmcv_cap2s as $hmcv_cap2)
        				<option value="{{$hmcv_cap2->id}}">{{$hmcv_cap2->tenhmcv}}</option>
        				@endforeach
        			</select>
        		</div>

        		<div class="form-group">
        			<label for="usr">Tên hạng mục cv cấp 3<span style="color: red;">(*)</span></label>
